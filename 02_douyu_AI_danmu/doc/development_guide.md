...
为了方便调试和配置共享，插件支持将配置（API Key, Prompt等）保存到本地 JSON 文件中。此功能采用了严格的“选择现有”与“新建/保存”分离的策略，以防止误操作导致数据覆盖。

1.  **`options.html` & `options.js`**:
    *   提供独立设置页面，通过 `popup.html` 右上角的设置按钮进入。
    *   使用 **File System Access API** 进行文件操作。
    *   **选择文件**：点击“选择文件”按钮，调用 `showOpenFilePicker`。此操作仅允许用户选择**已存在的** JSON 文件，不会触发覆盖警告，也不会修改文件内容。
    *   **新建/保存文件**：
        *   若未选择文件，点击“保存到文件”会调用 `showSaveFilePicker`（相当于“另存为”），允许用户输入新文件名创建一个新的配置文件。
        *   若已选择文件，点击“保存到文件”会将当前配置写入该文件。
    *   **从文件读取**：点击“从文件读取”按钮，将选中文件的内容加载到当前插件配置中。
    *   **自动同步 (Auto-Sync)**：开启后，插件会在启动时自动从选中的文件加载配置，并在配置发生更改时自动写入该文件，实现双向绑定。
2.  **`file_manager.js`**:
    *   封装文件系统操作。
...
### 2026-01-24 (持续优化)
*   **逻辑优化**: 改进了外部配置的交互流程，将“打开现有文件”与“新建/保存文件”逻辑分离，解决了选择文件时的覆盖警告问题。
*   **交互改进**: “选择文件”按钮现在仅用于指定文件，不再自动触发读写，操作权完全交给用户。
*   **代码规范**: 对所有核心 JS 文件进行了注释中文化处理，提高了代码的可读性。
*   **文档更新**: 同步更新了开发指南和使用说明，详细说明了新的文件管理机制。
