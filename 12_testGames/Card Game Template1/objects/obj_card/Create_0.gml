	...
	if !(obj_mouse_manager.grabbed_card == self) 
	{
		if !(picked_up)
		{
			...
			if (x < goal_x + 2) && (x > goal_x - 2) && (y < goal_y + 2) && (y > goal_y - 2)
			{
				...
				{
					var _card = stack.card_holder[_i];
					
					// Only update depth for cards that are settled at their goal.
					// Grabbed, carried, and snap-back cards manage their own depth while travelling
					var _at_goal = (_card.x > _card.goal_x - 2) && (_card.x < _card.goal_x + 2)
						&& (_card.y > _card.goal_y - 2) && (_card.y < _card.goal_y + 2);
						
					if (!(obj_mouse_manager.grabbed_card == _card) && !(_card.picked_up) && _at_goal)
					{
						_card.depth = _card.act_depth;	
					}
				}
			}
		}
